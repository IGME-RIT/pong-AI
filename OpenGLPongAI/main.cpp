...
glm::vec3 gballDirection = glm::normalize(glm::vec3(1, -1, 0));
...
//Simple Pong AI that moves based on ball y position
void AIController(Transform* aiPlayer, Transform* ghostBall, float dt)
{
	float mod = 0.04f;

	//Only move if ball is coming to us
	if (ballDirection.x < 0)
		return;

	if (ghostBall->position.y + mod > aiPlayer->position.y)
	{
		aiPlayer->position.y +=  playerVelocity * dt * 0.8f;
	}
	else if (ghostBall->position.y - mod < aiPlayer->position.y)
	{
		aiPlayer->position.y -= playerVelocity * dt * 0.8f;
	}
...
#pragma region Game_Setup
	...
	//This is a ball copy but a few frames ahead
	Transform* ghostBall = new Transform();
	ghostBall->position = glm::vec3(0.0, 0.8, 0.0);
	ghostBall->scale = glm::vec3(ballWidth, ballHeight, 1.0);
	ghostBall->Update();
...
#pragma region Ball_Updates
		...
		//update ball copy
		if (ghostBall->position.x < player2->position.x && ghostBall->position.x > player1->position.x)
			ghostBall->position += gballDirection * ballVelocity * dt * 1.2f;

		if (ghostBall->position.y >= 1 || ghostBall->position.y <= -1)
			gballDirection.y *= -1;
		...
		if (AABBCollision(p1Box, ballBox))
		{
			...
			gballDirection = ballDirection;
			ghostBall->position = ball->position;
	
		}
		else if (AABBCollision(p2Box, ballBox))
		{
		...
		//AI player Update based on ball copy
		//This makes it look the AI is predicting
		//where the ball is going.
		AIController(player2, ghostBall, dt);
